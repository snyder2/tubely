...
func (cfg *apiConfig) handlerVideoGet(w http.ResponseWriter, r *http.Request) {
	...
	finalVideo, err := cfg.dbVideoToSignedVideo(video)
	if err != nil {
		respondWithError(w, 500, "Failed to create presigned URL for video", err)
	}
...
func (cfg *apiConfig) handlerVideosRetrieve(w http.ResponseWriter, r *http.Request) {
	...
	for i, video := range videos {
		...
		if err != nil {
			respondWithError(w, 500, "Failed to generate presigned URL for video", err)
		}
		videos[i] = newVideo
		fmt.Println("Video URL:", newVideo.VideoURL)
	}
